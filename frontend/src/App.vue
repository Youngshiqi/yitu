...
const shipmentForm = ref({ sender_address_id: '', receiver_address_id: '', pickup_method: 'DOOR_PICKUP', delivery_method: 'HOME_DELIVERY', package_category: '日用品', package_description: '', estimated_weight_grams: 1000, estimated_length_cm: 30, estimated_width_cm: 20, estimated_height_cm: 20, special_instructions: '' })
...
const confirmingAgentShipment = ref(false)
...
const unreadNotificationCount = computed(() => notifications.value.filter(n => n.status !== 'READ').length)
...
const nav = computed(() => [
  ...
  { id: 'notifications', label: '消息中心', icon: Bell, badge: unreadNotificationCount.value },
])
...
async function confirmAgentShipment() { if (!activeConversation.value || confirmingAgentShipment.value) return; confirmingAgentShipment.value = true; try { const grant = await issueAgentGrant(activeConversation.value.id); await consumeAgentGrant(grant.id); agentDraft.value = await getAgentDraft(activeConversation.value.id); agentMessages.value = await listMessages(activeConversation.value.id); ElMessage.success('运单已创建'); await loadData() } catch { ElMessage.error('草稿尚未准备好，请重新确认报价') } finally { confirmingAgentShipment.value = false } }
    ...
    <main class="main"><header class="topbar"><div><div class="crumb">客户中心 <span>/</span> {{ nav.find(n => n.id === view)?.label ?? '运单详情' }}</div><h1>{{ view === 'detail' ? '运单详情' : nav.find(n => n.id === view)?.label }}</h1></div><div class="top-actions"><el-input v-model="query" placeholder="搜索运单号" :prefix-icon="Search" clearable /><el-avatar :size="34">{{ user?.display_name?.slice(0, 1) }}</el-avatar><span class="user-name">{{ user?.display_name || '演示客户' }}</span></div></header>
      <section v-loading="loading" class="content">
        ...
        <div v-else-if="view === 'agent'" :class="['agent-page', { 'chat-list-collapsed': chatListCollapsed }]">
          ...
          <section class="chat-window"><div v-if="activeConversation" class="chat-body"><div class="chat-intro"><div class="agent-orb"><ChatDotRound /></div><h3>你好，我是 Yitu 寄件助手</h3><p>告诉我寄件和收件城市、物品重量，我会帮你准备运单草稿并计算报价。</p></div><div ref="messagesContainer" class="messages"><div v-for="message in agentMessages" :key="message.id" :class="['message', message.role]"><div v-if="message.role === 'assistant'" class="message-bubble markdown-body"><div v-if="message.content" v-html="renderMarkdown(message.content)" /><div v-else class="typing-indicator" role="status" aria-label="AI 正在输入"><span></span><span></span><span></span></div></div><div v-else class="message-bubble">{{ message.content }}</div></div><div v-if="agentDraft?.status === 'READY_FOR_CONFIRMATION'" class="message assistant"><div class="message-bubble"><strong>运单明细</strong><table><tr v-for="item in agentDraft.summary || []" :key="item.label"><th>{{ item.label }}</th><td>{{ item.value }}</td></tr></table><el-button type="primary" :loading="confirmingAgentShipment" @click="confirmAgentShipment">确认下单</el-button></div></div></div><div class="chat-composer"><el-input v-model="agentInput" type="textarea" :rows="2" resize="none" placeholder="例如：帮我从广州寄一箱衣服到上海" @keydown.enter.exact.prevent="sendMessage" /><el-button type="primary" :loading="agentSending" :icon="ArrowRight" @click="sendMessage" /></div></div><div v-else class="chat-empty"><div class="agent-orb"><ChatDotRound /></div><h3>用对话完成寄件</h3><p>从地址、重量到报价确认，Yitu 助手会一步步帮你完成。</p><el-button type="primary" @click="startNewConversation">开始新对话</el-button></div></section>
        </div>
        ...
        <div v-else-if="view === 'notifications'" class="page-block"><div class="section-head"><div><p class="section-kicker">NOTIFICATIONS</p><h2>消息中心</h2></div><el-button text type="primary" :disabled="!unreadNotificationCount" @click="readAllNotices">全部标为已读</el-button></div><div class="notice-list"><div v-for="n in notifications" :key="n.id" :class="['notice', { unread: n.status !== 'READ' }]" @click="readNotice(n)"><div class="notice-dot"></div><div><strong>{{ n.title }}</strong><p>{{ n.content }}</p><el-button v-if="n.template_code === 'SUPPLEMENT_REQUIRED'" type="primary" size="small" @click.stop="confirmSupplement(n)">确认补缴 ¥{{ (supplementAmount(n) / 100).toFixed(2) }}</el-button><time>{{ new Date(n.created_at).toLocaleString('zh-CN') }}</time></div></div><el-empty v-if="!notifications.length" description="暂无消息" /></div></div>
        <div v-else class="page-block"><el-button text @click="view = 'shipments'">← 返回运单</el-button><div v-if="selected?.shipment" class="detail-layout"><div class="detail-main"><p class="section-kicker">SHIPMENT DETAIL</p><h2>{{ selected.shipment.shipment_no }}</h2><el-tag type="primary">{{ statusMap[selected.shipment.status] || selected.shipment.status }}</el-tag><div class="shipment-times"><span>下单时间：{{ new Date(selected.shipment.created_at).toLocaleString('zh-CN') }}</span><span>支付时间：{{ selected.paid_at ? new Date(selected.paid_at).toLocaleString('zh-CN') : '尚未支付' }}</span></div><div class="route-summary"><div><small>寄件信息</small><strong>{{ selected.sender_address?.recipient_name || '未提供' }}</strong><p>{{ selected.sender_address?.full_address || '暂无寄件地址' }}</p></div><ArrowRight /><div><small>收件信息</small><strong>{{ selected.receiver_address?.recipient_name || '未提供' }}</strong><p>{{ selected.receiver_address?.full_address || '暂无收件地址' }}</p></div></div><div v-if="selected.package" class="package-summary"><div><small>托寄物</small><strong>{{ selected.package.category }}</strong><p>{{ selected.package.description }}</p></div><div><small>预估重量</small><strong>{{ (selected.package.estimated_weight_grams / 1000).toFixed(2) }} kg</strong><p>{{ selected.package.estimated_length_cm }} × {{ selected.package.estimated_width_cm }} × {{ selected.package.estimated_height_cm }} cm</p></div><div><small>运费</small><strong>¥{{ ((selected.quote?.total_cents || 0) / 100).toFixed(2) }}</strong><p>{{ selected.shipment.status === 'PENDING_PAYMENT' ? '待支付' : `已支付 ¥${(selected.paid_total_cents / 100).toFixed(2)}` }}</p><div v-if="selected.quote?.fee_items?.length" class="fee-breakdown"><span v-for="item in selected.quote.fee_items" :key="item.code">{{ feeItemLabel(item.code) }}：¥{{ (item.amount_cents / 100).toFixed(2) }}</span></div></div></div><div class="timeline"><div v-for="event in timeline" :key="event.id" class="timeline-item"><div class="timeline-dot"></div><div class="timeline-content"><span class="timeline-text">{{ renderTrackingMessage(event.message) }}</span><time>{{ new Date(event.occurred_at).toLocaleString('zh-CN') }}</time></div></div><el-empty v-if="!timeline.length" description="暂无轨迹" /></div></div></div><el-empty v-else description="暂无运单详情" /></div>
      </section>
    </main>
...
/* AI 回复首个 token 到达前，用三点跳动动画提示「正在思考」，缓解等待焦虑。 */
.typing-indicator {
  display: inline-flex;
  align-items: center;
  gap: 4px;
...
.timeline-item { display: grid; grid-template-columns: 14px minmax(260px, 1fr); gap: 14px; align-items: start; padding: 14px 16px; border: 1px solid #dce3de; background: #f9faf8; }
.timeline-content { width: 100%; min-width: 0; }
.timeline-dot { width: 10px; height: 10px; margin-top: 5px; border-radius: 50%; background: #b18c50; box-shadow: 0 0 0 4px #f2e8d8; }
.timeline-text { display: block; width: 100%; min-width: 0; color: #25332e; font-size: 14px; line-height: 1.6; white-space: normal; word-break: keep-all; writing-mode: horizontal-tb; }
.timeline-item time { display: block; margin-top: 4px; color: #7d8c85; font-size: 12px; white-space: nowrap; }
