...
async function confirmAgentShipment() { if (!activeConversation.value || confirmingAgentShipment.value) return; confirmingAgentShipment.value = true; try { const grant = await issueAgentGrant(activeConversation.value.id); await consumeAgentGrant(grant.id); agentDraft.value = await getAgentDraft(activeConversation.value.id); ElMessage.success('运单已创建'); await loadData(); view.value = 'shipments' } catch { ElMessage.error('草稿尚未准备好，请重新确认报价') } finally { confirmingAgentShipment.value = false } }
