...
export interface CurrentUser {
  id: string; display_name: string; role: Role; station_id: string | null
}
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(loginName: string, password: string) {
    ...
    try {
      const data = await api.post<{ access_token: string; token_type: string }>('/auth/demo-login', { login_name: loginName, password })
      localStorage.setItem('yitu_token', data.access_token)
      token.value = data.access_token
      await fetchMe()
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : '登录失败'
      error.value = msg; throw err
  ...
  async function fetchMe() {
    ...
    try { user.value = await api.get<CurrentUser>('/auth/me') }
    catch { logout() }
  }

  return { user, token, loading, error, login, logout, fetchMe }
})