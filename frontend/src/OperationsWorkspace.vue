...
import { Box, Clock, Connection, DataAnalysis, Delete, Edit, Files, OfficeBuilding, Plus, Refresh, Setting, TrendCharts, Warning } from '@element-plus/icons-vue'
import { applyExceptionAction, arriveDestination, createAdminStation, createSlaRule, deleteAdminStation, listAdminStations, listCourierTasks, listExceptions, listRegions, listShipments, listSlaInstances, listSlaRules, resolveException, setAdminStationEnabled, updateAdminStation, type AdminStation, type CourierTask, type ExceptionCase, type ServiceType, type Shipment, type SLARule } from './api'
...
async function loadData() {
  loading.value = true
  try { const [shipmentResult, taskResult, caseResult, ruleResult] = await Promise.all([listShipments({ limit: 50, offset: 0 }), listCourierTasks(), listExceptions({ limit: 100, offset: 0 }), listSlaRules()]); shipments.value = shipmentResult.items || []; totalShipments.value = shipmentResult.total || 0; tasks.value = taskResult; cases.value = caseResult.items; slaRules.value = ruleResult } catch { ElMessage.error('运营数据加载失败，请确认后端服务已启动') } finally { loading.value = false }
}
