...
import { Box, Clock, Connection, DataAnalysis, Delete, Edit, Files, Money, OfficeBuilding, Plus, Refresh, Setting, TrendCharts, Warning } from '@element-plus/icons-vue'
import { applyExceptionAction, arriveDestination, createAdminStation, createPricingRule, createSlaRule, deleteAdminStation, listAdminStations, listCourierTasks, listExceptions, listPricingRules, listRegions, listShipments, listSlaInstances, listSlaRules, resolveException, setAdminStationEnabled, updateAdminStation, type AdminStation, type CourierTask, type ExceptionCase, type PricingRule, type ServiceType, type Shipment, type SLARule } from './api'
...
const nav = [{ id: 'overview', label: '运营概览', icon: DataAnalysis }, { id: 'shipments', label: '全部运单', icon: Box }, { id: 'exceptions', label: '异常工单', icon: Warning }, { id: 'fulfillment', label: '履约调度', icon: Connection }, { id: 'stations', label: '网点管理', icon: OfficeBuilding }, { id: 'sla', label: 'SLA 监控', icon: TrendCharts }, { id: 'pricing', label: '运费规则', icon: Money }, { id: 'knowledge', label: '知识库', icon: Files }, { id: 'deadletters', label: '死信队列', icon: Warning }, { id: 'retrieval', label: '检索验证', icon: Files }]
...
async function loadData() {
  loading.value = true
  try { const [shipmentResult, taskResult, caseResult, ruleResult, pricingRuleResult] = await Promise.all([listShipments({ limit: 50, offset: 0 }), listCourierTasks(), listExceptions({ limit: 100, offset: 0 }), listSlaRules(), listPricingRules()]); shipments.value = shipmentResult.items || []; totalShipments.value = shipmentResult.total || 0; tasks.value = taskResult; cases.value = caseResult.items; slaRules.value = ruleResult; pricingRules.value = pricingRuleResult } catch { ElMessage.error('运营数据加载失败，请确认后端服务已启动') } finally { loading.value = false }
}
...
async function saveSlaRule() { const form = slaRuleForm.value; if (!form.version.trim() || !form.route_code.trim() || !form.stage.trim() || form.target_work_hours <= 0) { ElMessage.warning('请完整填写 SLA 版本、线路、阶段和时长'); return }; try { const rule = await createSlaRule({ ...form }); slaRules.value = [rule, ...slaRules.value]; slaRuleDialog.value = false; ElMessage.success('SLA 规则已发布') } catch (error: any) { ElMessage.error(error.response?.data?.message || 'SLA 规则发布失败') } }
...
<template>
  <div class="app-shell operations-shell"><aside class="sidebar"><div class="logo"><span>Y</span><div>Yitu<small>物流工作台</small></div></div><div class="workspace-label">运营管理工作区</div><nav><button v-for="item in nav" :key="item.id" :class="{ active: view === item.id }" @click="changeView(item.id)"><component :is="item.icon" /><span>{{ item.label }}</span><b v-if="item.id === 'exceptions' && activeCases.length">{{ activeCases.length }}</b></button></nav><div class="ops-status"><small>全网状态</small><strong>运行稳定</strong><span><i></i> 事件队列已同步</span></div><div class="sidebar-foot"><el-button text @click="$emit('logout')"><Setting /> 退出登录</el-button></div></aside>
    <main class="main"><header class="topbar"><div><div class="crumb">运营中心 <span>/</span> {{ nav.find(item => item.id === view)?.label }}</div><h1>{{ nav.find(item => item.id === view)?.label }}</h1></div><div class="top-actions"><el-input v-model="query" placeholder="搜索运单或异常说明" clearable /><el-button circle :icon="Refresh" @click="loadData" /><el-avatar :size="34">{{ user.display_name.slice(0, 1) }}</el-avatar><span class="user-name">{{ user.display_name }}</span></div></header>
      <section v-loading="loading" class="content"><SystemAdminWorkspace v-if="['knowledge', 'deadletters', 'retrieval'].includes(view)" :user="user" :initial-view="view" embedded /><div v-else class="page-block">
