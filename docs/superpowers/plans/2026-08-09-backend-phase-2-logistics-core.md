...
- [x] 测试客户所有权、运单号不可变、相同幂等键重放和不同请求哈希冲突。
- [x] 运行测试确认红灯。
- [x] 实现创建事务、幂等服务调用、初始状态和 Outbox 追加；不得直接允许任意状态写入。
- [x] 运行迁移和创建测试。
- [x] 提交：`功能：实现运单聚合与幂等创建`。
...
- [x] 测试合法初始转换、非法跳转、重复事件和轨迹顺序。
- [x] 运行测试确认红灯。
- [x] 实现显式转换表；状态、轨迹、审计在同一事务写入。
- [x] 运行迁移与状态机测试。
- [x] 提交：`功能：实现运单状态机与轨迹记录`。
...
- [x] 测试上门揽收创建任务、网点自寄不创建揽收任务、任务归属和始发到站动作。
- [x] 运行测试确认红灯。
- [x] 实现角色/网点检查、状态转换、轨迹和审计；接单并发放在下一个小任务。
- [x] 运行迁移与聚焦测试。
- [x] 提交：`功能：新增始发端揽收与自寄任务`。
...
- [x] 编写两个并发请求争抢同一任务的测试：一个返回 200，另一个返回 `409 TASK_ALREADY_ASSIGNED`。
- [x] 运行并发测试确认红灯。
- [x] 使用 `UPDATE ... WHERE status=AVAILABLE RETURNING` 和数据库事务锁实现原子接单。
- [x] 连续运行并发测试 3 次，确认结果稳定。
- [x] 提交：`并发：保证快递员任务原子接单`。
...
- [x] 测试快递员所有权、一次签收证明、六位取件码哈希、五次错误锁定、过期、补发和成功幂等重放。
- [x] 运行末端测试确认红灯。
- [x] 使用 Argon2id + pepper 和原子尝试/消费更新实现派送、自取和终态转换。
- [x] 运行迁移与末端测试，确认明文凭证不出现在响应日志。
- [x] 提交：`安全：完成派送与网点自取凭证`。
