# Yitu Backend Program Implementation Roadmap
...
**Goal:** 按六个可验收阶段完成 Yitu 全部后端，冻结 OpenAPI 后再开始前端。

**Architecture:** FastAPI 模块化单体承载同步业务，PostgreSQL/pgvector 保存长期事实，Redis/Celery 处理异步任务，LangGraph 编排 Agent。各阶段只通过稳定应用服务协作，普通 API 与 Agent 工具不得复制领域规则。

**Tech Stack:** Python 3.11、FastAPI、Pydantic v2、SQLAlchemy 2、Alembic、PostgreSQL 16、pgvector、Redis 7、Celery、LangGraph、MinerU、PyMuPDF、pytest、HTTPX、Ruff、mypy、Docker Compose。
...
| 设计范围 | 执行计划 |
|---|---|
| 平台、错误、时间、数据库、幂等、审计、Outbox | Phase 1 |
| 身份、网点、运单、任务、干线、轨迹、签收 | Phase 2 |
| 计价、支付、SLA/ETA、通知、异常、退回、恢复 | Phase 3 |
| 文件、解析、发布、索引、检索、引用 | Phase 4 |
| 对话、双入口下单、工具、授权、记忆、隐私、评测 | Phase 5 |
| 迁移、演示、安全、性能、OpenAPI 和前端契约 | Phase 6 |

PRD 中的每项后端验收标准至少映射到上表一个阶段；涉及多个模块的旅程在 Phase 6 再执行一次全量回归。
