...
- [x] 提交：`功能：新增版本化运单计价`；后续补充提交修正规则生效时间过滤。
...
- [x] 测试重复回调、少付/多付拒绝、复重补差门槛、揽收前取消和全额退款。
- [x] 运行支付测试；预期因缺少服务而失败。
- [x] 实现只追加的支付交易和幂等模拟回调；由运单服务消费支付结果。
- [x] 运行迁移以及支付/运单测试；预期全部通过。
- [x] 提交：`功能：新增模拟支付生命周期`。
...
- [x] 测试工作时间边界、UTC+8、冻结承诺、暂停补差时间、超时扫描幂等和 ETA 分离。
- [x] 运行 SLA 测试；已确认缺少模块时测试失败。
- [x] 实现版本化规则、阶段实例、注入式 Clock 和幂等扫描窗口。
- [x] 运行迁移和 SLA 测试；已全部通过。
- [x] 提交：`功能：跟踪运单 SLA 与 ETA`。
...
- [x] 测试模板白名单、逻辑/渠道状态、重复 Outbox 事件、重试、模拟短信和 SSE 重连游标。
- [x] 运行通知测试；已确认缺少模块时测试失败，并在实现后覆盖关键路径。
- [x] 实现不调用模型的模板渲染、投递幂等键 `(event, recipient, channel)` 和有界 SSE 心跳。
- [x] 运行迁移和通知测试；通知模块 13 个测试通过。
- [x] 提交：`功能：交付可靠通知`。
...
- [x] 测试全部异常状态、网点/角色范围、履约暂停、显式恢复目标和审计原因。
- [x] 运行异常测试；已在实施过程中覆盖预期失败与关键回归。
- [x] 实现独立的异常单状态机，并调用运单、调度和 SLA 应用服务，不直接修改其他模块的模型。
- [x] 运行迁移和异常测试；最终使用 `0018_create_exceptions.py`，异常模块 31 个测试通过。
- [x] 提交：`功能：新增可审计异常处理`；任务五拆分为多个聚焦提交，最终收口提交记录完整验收结果。
...
- [x] 测试支付前/后取消、揽收后拦截、重派、新任务/SLA、转自取、退回运输和退款影响。
- [x] 运行恢复旅程测试；已用最小 HTTP 旅程覆盖任务六核心动作。
- [x] 使用新的运输段和任务实现退回生命周期，并只追加轨迹、审计和支付影响。
- [x] 运行迁移和恢复测试；最终使用 `0019_create_returns.py`，恢复旅程测试通过。
- [x] 提交：`功能：闭环取消与退回流程`；任务六拆分为核心动作与 SLA 衔接两个提交。
