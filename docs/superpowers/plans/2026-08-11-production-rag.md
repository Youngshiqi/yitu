...
- [x] 增加最小单元测试，验证对象键、内容类型、签名有效期和本地实现不支持签名时的稳定异常。
- [x] 扩展 `BlobStore` 协议；`S3BlobStore` 使用 `generate_presigned_url("get_object", ...)`，默认有效期 900 秒。
- [x] 让 PDF、Markdown、JSON、ZIP、图片共用 `put`，调用方显式传递 MIME。
- [x] 运行 `uv run pytest tests/knowledge/test_blob_store.py -q`、Ruff、mypy。
- [x] 提交 `功能：扩展 COS 知识产物存储能力`。
...
- [x] 用 `httpx.MockTransport` 覆盖提交成功、处理中、完成、限流、5xx 和永久错误。
- [x] 实现 Bearer 认证、`POST /api/v4/extract/task`、`GET /api/v4/extract/task/{task_id}` 和 `full_zip_url` 下载。
- [x] 定义稳定异常 `MinerURetryableError`、`MinerUPermanentError`，异常中不包含 Token 或签名 URL。
- [x] 运行相关测试、Ruff、mypy并提交 `功能：接入 MinerU 云端解析 API`。
...
- [x] 增加模型和迁移断言，要求 MinerU 任务 ID 可恢复且产物对象键可追踪。
- [x] 添加字段、索引和响应字段；历史迁移保持不变。
- [x] 运行 `alembic heads`、相关测试、Ruff、mypy并提交 `功能：持久化 MinerU 解析工作流`。
...
- [x] 覆盖重复投递、Worker 重启、处理中重试、失败状态和成功产物回存。
- [x] 安全解压 ZIP：限制 200MB、2000 个文件、拒绝绝对路径和 `..`，读取 `full.md`。
- [x] 提交任务时生成 15 分钟 COS URL；持久化 task_id 后独立轮询，完成后回存 ZIP 与 Markdown。
- [x] 永久错误进入 `PARSE_FAILED`，临时错误使用 Celery 退避重试；日志仅记录 document_id/task_id。
- [x] 运行相关测试并提交 `功能：实现可恢复的 MinerU 解析 Worker`。
...
- [x] 使用 OpenAI 兼容 MockTransport 覆盖批量输入、顺序保持、维度探测、429、5xx、空响应和维度漂移。
- [x] 实现批次大小 20、超时 60 秒、重试后稳定异常；生产配置选择 Qwen，测试配置选择确定性 Provider。
- [x] 响应向量归一化并记录模型名与维度，不记录文本内容。
- [x] 运行相关测试并提交 `功能：接入 Qwen 文本向量模型`。
...
- [x] 增加迁移测试，验证 `CREATE EXTENSION vector`、JSONB 旧向量清理策略和新列约束。
- [x] 使用 `pgvector` SQLAlchemy 类型；维度按 Qwen 首次探测结果固定，维度不一致拒绝构建索引。
- [x] 创建适合当前规模的 HNSW cosine 索引，保留关键词 GIN 索引。
- [x] 运行迁移往返、相关测试、Ruff、mypy并提交 `功能：启用 pgvector 知识向量索引`。
...
- [x] 覆盖标题边界、Markdown 表格、页码标记、800 字符上限、100 字符重叠和版本隔离。
- [x] 解析 MinerU Markdown 标题与表格，生成确定性 chunk_id；按最多 20 条批量生成并写入向量。
- [x] 索引构建失败时保留上一可用版本，不发布半成品。
- [x] 运行相关测试并提交 `功能：构建生产级版本化知识索引`。
- [ ] 适配 MinerU 结构化布局产物，把真实 PDF 页码和区块坐标映射到知识块。
...
- [x] 覆盖中文查询、分类、生效时间、未发布排除、停用排除和证据页码字段。
- [x] 在 PostgreSQL 中计算 cosine distance 与全文排名，归一化后按关键词 0.55、向量 0.45 融合。
- [x] 限制候选集和最终结果数，分数相同时使用稳定顺序。
- [x] 运行相关测试并提交 `功能：实现 pgvector 混合知识检索`。
...
- [x] 增加固定适配器的自动化旅程：上传、MinerU 完成、审核、发布、检索。
- [x] 使用本机 `.env` 单独运行 COS put/get/delete、MinerU 提交/轮询、Qwen 维度探测冒烟脚本；不打印密钥和签名 URL。
- [x] 使用真实 Compose 执行完整 HTTP 流程，并记录文档 ID、解析状态、模型、维度和证据数量。
...
- [x] 用中文记录 COS、MinerU、Qwen、pgvector 配置与密钥轮换步骤，示例只用空值。
- [x] 记录解析失败、Worker 重启、模型维度变化和索引重建流程。
- [x] 按用户要求不运行 pytest；运行 `uv run ruff check .`、`uv run mypy src`、迁移状态检查和 `docker compose config`。
- [x] 确认 Git 工作区无密钥、签名 URL、PDF 或解析产物，提交 `文档：完成生产级 RAG 阶段验收`。
