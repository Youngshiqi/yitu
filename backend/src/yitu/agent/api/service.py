...
class AgentConversationService:
    ...
    def __init__(
        self, session: AsyncSession, runner: AgentGraphRunner | None = None
    ) -> None:
    ...
    async def list_conversations(self, actor: CurrentUser) -> list[AgentConversation]:
        rows = await self._session.scalars(
            ...
            .where(
                ...
                exists().where(AgentMessage.conversation_id == AgentConversation.id),
            )
    ...
    async def send_message(
        self, conversation_id: UUID, content: str, context: AgentRuntimeContext
    ) -> AgentTurnView:
        return await self._require_runner().invoke_message(
    ...
    async def save_draft_address(
        self, conversation_id: UUID, actor: CurrentUser, payload: DraftAddressCreate
    ) -> DraftView:
        await self.get_owned(conversation_id, actor)
        ...
        return await DraftService(self._session).update(
            ...
            DraftPatch(
                ...
                origin_district_code=address.district_code
                if payload.role == "sender"
                else None,
                destination_district_code=address.district_code
    ...
    async def record_consumption_receipt(
        self, grant_id: UUID, actor: CurrentUser, shipment: ShipmentView
    ) -> None:
    ...
    async def delete_conversation(
        self, conversation_id: UUID, actor: CurrentUser, request_id: str
    ) -> None:
        ...
        await AuditService(self._session).record(
            ...
            before_summary={"conversation_id": str(row.id)},
